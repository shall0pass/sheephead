...
+ IndexedDB storage). The sync server is a dumb relay + durable store.
...
interface GameDoc {
	...
	handsToPlay: number; // game ends after this many scored hands (default 10)
...
| `play.ts`     | Trick 1 is led by the eldest hand (`dealer + 1`), **not** the picker. `legalMoves(doc, seat)`: **follow the led suit** (trump counts as one suit — a led Queen/Jack/Diamond means "trump was led"); if void in the led suit, **any card is legal** (Sheephead has *no* forced trump‑in and *no* overtrump obligation, unlike Clabber). Called‑ace constraints (both lift on the last trick): the **partner** must play the called ace on the first lead of its suit and may **not** slough it on any other trick; the **picker** must keep at least one fail card of the called suit and may not play it except when that suit is led; an `under` card is played face‑down when the called suit is led, is seen only by the trick winner, and never wins the trick. `applyPlay`, `resolveTrick` (highest trump, else highest card of the led fail suit — no last‑trick bonus), winner leads next. |
...
| `state.ts`    | Seat helpers, `pickerTeam(doc)` / `oppTeam(doc)` derived from `picker` + `partnerSeat`, `createGame({ code })` with 5 seats and `handsToPlay: 10`.                                                                                                                                                                                                                  |
| `actions.ts`  | Action union: `JoinSeat`, `LeaveSeat`, `RenameSeat`, `SetBot`, `CoverSeat`, `StartHand`, `Pick`, `Pass`, `Bury`, `CallPartner` (`{suit} | {under, suit, card} | {alone}`), `PlayCard`, `ResetToLobby`, `HostClaim`, `SendChat`. **Dropped from Clabber:** `Bid`, `AnnounceMeld`, `ShowMeld`.                                                                          |
...
- **Call partner:** call the ace of the fail suit the bot is *shortest* in while
...
> and accepted at almost all tournaments" — when the picking team *loses*, its
...
> **Open choice — game length & "winning team".** Sheephead has no natural 500
> target. v1 plays a fixed `handsToPlay` (default 10 = two deals each) and then
> splits on the sign of each player's cumulative tally. Alternatives to confirm
> with the stakeholder: play to a target score, play a set wall‑clock time, or
> let the lobby pick the hand count.
...
- **All‑pass resolution.** `artifacts/game_rules.md` says the hand "becomes a
  ...
  aren't on the critical path. Two guards are needed: the dealer bot must *pick*
...
- **Trust model.** Hands *and the hidden partner* are readable in the raw doc.
