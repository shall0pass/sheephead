...
import { choosePick, chooseBury, chooseCall, chooseCard } from './bot';
...
export function nextBotAction(
	...
	switch (doc.phase) {
		case 'picking': {
			...
			return choosePick(doc, seat) ? { type: 'Pick', seat } : { type: 'Pass', seat };
		}
		case 'bury': {
			const seat = doc.picker;
			if (seat == null || !doc.players[seat]?.isBot) return null;
			return { type: 'Bury', seat, cards: chooseBury(doc) };
		}
		case 'callPartner': {
			const seat = doc.picker;
			if (seat == null || !doc.players[seat]?.isBot) return null;
			return { type: 'CallPartner', seat, call: chooseCall(doc) };
		}
		case 'trick': {
